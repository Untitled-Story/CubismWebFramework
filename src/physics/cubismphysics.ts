...
export class CubismPhysics {
    ...
    while (this._currentRemainTime >= physicsDeltaTime) {
      ...
      for (
        ...
        for (let i = 0; i < currentSetting.outputCount; ++i) {
          this._previousRigOutputs[settingIndex].output[i] =
            this._currentRigOutputs[settingIndex].output[i];
        }
      }
      ...
      for (
        ...
        for (let i = 0; i < currentSetting.outputCount; ++i) {
          ...
          this._currentRigOutputs[settingIndex].output[i] = outputValue;
    ...
    for (
      ...
      for (let i = 0; i < currentSetting.outputCount; ++i) {
        ...
        updateOutputParameterValue(
          ...
          parameterMinimumValue[destinationParameterIndex],
          parameterMaximumValue[destinationParameterIndex],
          this._previousRigOutputs[settingIndex].output[i] * (1 - weight) +
            this._currentRigOutputs[settingIndex].output[i] * weight,
          currentOutput[i]
        );
  ...
  _currentRigOutputs: PhysicsOutput[]; ///< 最新の振り子計算の結果
  _previousRigOutputs: PhysicsOutput[]; ///< 一つ前の振り子計算の結果
