...
export class CubismRenderTextureResource {
  ...
  public textures: WebGLFramebuffer[]; // テクスチャのアドレス
}
...
export class CubismRenderer_WebGL extends CubismRenderer {
  ...
  public release(): void {
    const self = this as Partial<this>;
    if (this._clippingManager) this._clippingManager.release();
  ...
  /**
   * 与えられた 4x4 変換行列に基づいて WebGL のカリング用 frontFace を更新する。
   * 行列式が負の場合は CW、正の場合は CCW に設定し、スケーリングや鏡像変換があっても正しい剔除方向を維持する。
   * @param matrix 剔除方向を判定するための CubismMatrix44 オブジェクト
   */
  private _updateFrontFaceByMatrix(matrix: CubismMatrix44) {
    const array = matrix.getArray();
    const det = array[0] * array[5] - array[1] * array[4];
    this.gl.frontFace(det < 0 ? this.gl.CW : this.gl.CCW);
  }
  ...
  public drawMesh(
    ...
    if (this.isCulling()) {
      ...
      this._updateFrontFaceByMatrix(this.getMvpMatrix());
    } else {
